...
function makeSymbolGeoJSON(calcTrace, gd) {
    ...
    for(var i = 0; i < calcTrace.length; i++) {
        ...
        if(txt) {
            txt = Array.isArray(txt) ? txt[i] : txt;
            calcPt.text = calcPt.tx;
            calcPt.lon = calcPt.lonlat[0];
            calcPt.lat = calcPt.lonlat[1];
            calcPt.customdata = calcPt.data;
            calcPt.txt = text = Lib.texttemplateString(txt, {}, gd._fullLayout._d3locale, calcPt, trace._meta || {});
        }
        ...
        features.push({
            ...
            properties: {
                ...
                text: fillText(calcPt.txt || calcPt.tx)
            }
        });
    }
